...
const runTests = async () => {
  try {
    ...
    if (regSender.status !== 201 && regSender.status !== 200)
      throw new Error(`Sender Reg Failed: ${JSON.stringify(regSender.data)}`);
    console.log("✅ Sender Registered");

    console.log("👉 Registering Receiver...");
    const regReceiver = await request("POST", "/auth/register", RECEIVER);
    if (regReceiver.status !== 201 && regReceiver.status !== 200)
      throw new Error(
        `Receiver Reg Failed: ${JSON.stringify(regReceiver.data)}`,
      );
    console.log("✅ Receiver Registered");

    console.log("👉 Registering Agent...");
    const regAgent = await request("POST", "/auth/register", AGENT);
    if (regAgent.status !== 201 && regAgent.status !== 200)
      throw new Error(`Agent Reg Failed: ${JSON.stringify(regAgent.data)}`);
    ...
    const loginSender = await request("POST", "/auth/login", {
      email: SENDER.email,
      password: SENDER.password,
    });
    if (loginSender.status !== 200)
      throw new Error(
        `Sender Login Failed: ${JSON.stringify(loginSender.data)}`,
      );
    ...
    const loginReceiver = await request("POST", "/auth/login", {
      email: RECEIVER.email,
      password: RECEIVER.password,
    });
    if (loginReceiver.status !== 200)
      throw new Error(
        `Receiver Login Failed: ${JSON.stringify(loginReceiver.data)}`,
      );
    receiverId = loginReceiver.data.data.user._id;
    console.log("✅ Receiver Logged In");

    console.log("👉 Logging in Agent...");
    const loginAgent = await request("POST", "/auth/login", {
      email: AGENT.email,
      password: AGENT.password,
    });
    if (loginAgent.status !== 200)
      throw new Error(`Agent Login Failed: ${JSON.stringify(loginAgent.data)}`);
    console.log(
      "Agent Login Response:",
      JSON.stringify(loginAgent.data, null, 2),
    );
    ...
    const approveAgent = await request(
      "PATCH",
      `/users/${agentId}/approval`,
      { approvalStatus: "APPROVED" },
      adminToken,
    );
    if (approveAgent.status !== 200)
      throw new Error(
        `Agent Approval Failed: ${JSON.stringify(approveAgent.data)}`,
      );
    ...
    if (updateConfig.status !== 200)
      throw new Error(
        `Config Update Failed: ${JSON.stringify(updateConfig.data)}`,
      );
    ...
    await Wallet.findByIdAndUpdate(agentUser.wallet, {
      $inc: { balance: 10000 },
    });
    ...
    if (cashIn.status !== 200)
      throw new Error(`Cash In Failed: ${JSON.stringify(cashIn.data)}`);
    ...
    if (sendMoney.status !== 200)
      throw new Error(`Send Money Failed: ${JSON.stringify(sendMoney.data)}`);
    ...
    if (cashOut.status !== 200)
      throw new Error(`Cash Out Failed: ${JSON.stringify(cashOut.data)}`);
    ...
    if (withdraw.status !== 200)
      throw new Error(`Withdraw Failed: ${JSON.stringify(withdraw.data)}`);
    ...
    if (history.status !== 200)
      throw new Error(`History Failed: ${JSON.stringify(history.data)}`);
    console.log(
      `✅ History Fetched: ${history.data.data.data.length} transactions`,
    );
